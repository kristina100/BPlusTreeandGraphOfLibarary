...
void Read_Data_And_Insert() {
    ...
    while (1) {
        ...
        demo->book[0].ISBN = new_st[1];
//        char* value = (char*)malloc(sizeof(char) * new_len);
//        strcpy(value, new_st);
        keys[key_num++] = new_key;
        if (BPlusTree_Insert(new_key, new_pos, demo) == true) validRecords++; // for "ex-data.txt", valid = 9950138
    }
    free(buffer);
    buffer = NULL;
}
...
void modify_test() {
    ...
    for (i = 0; i < mod_num; i++) {
        ...
        scanf("%c,%s", &new_key, new_st);
//                char* value = (char*)malloc(sizeof(char) * strlen(new_st));
//                strcpy(value, new_st);
        BPTNode* demo;
        demo = (BPTNode*)malloc(sizeof(BPTNode));
        demo->book[0].Title = new_st;
